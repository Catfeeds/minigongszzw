...
class IndexController extends PublicController {
    ...
    public function index(){
        ...
        //======================
        $bottom = array();
        $bottom = M('server_company')->select();
        //======================
        //首页推荐人气商品
        //======================
    	...
    	// }
        $pro = array();
        $company = M('company_register')->limit(1)->select();
        if($company){
            foreach ($company as $k => $v) {
                $company[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$company);
        } 
        $agency = M('agency')->limit(1)->select();
        if($agency){
            foreach ($agency as $k => $v) {
                $agency[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$agency);
        }
        $affiliate = M('affiliate')->limit(1)->select();
        if($affiliate){
            foreach ($affiliate as $k => $v) {
                $affiliate[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$affiliate);
        }
        $hongkong = M('hongkong')->limit(1)->select();
        if($hongkong){
            foreach ($hongkong as $k => $v) {
                $hongkong[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$hongkong);
        }
        $shebao = M('shebao')->limit(1)->select();
        if($shebao){
            foreach ($shebao as $k => $v) {
                $shebao[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$shebao);
        }
        $gongsi_change = M('gongsi_change')->limit(1)->select();
        if($gongsi_change){
            foreach ($gongsi_change as $k => $v) {
                $gongsi_change[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$gongsi_change);
        }
        $person_register = M('person_register')->limit(1)->select();
        if($person_register){
            foreach ($person_register as $k => $v) {
                $person_register[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$person_register);
        }
        $special_approval = M('special_approval')->limit(1)->select();
        if($special_approval){
            foreach ($special_approval as $k => $v) {
                $special_approval[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$special_approval);
        }
        $subsidy = M('subsidy')->limit(1)->select();
        if($subsidy){
            foreach ($subsidy as $k => $v) {
                $subsidy[$k]['photo_x'] = __DATAURL__.$v['photo_x'];
            }
            $pro = array_merge($pro,$subsidy);
        }  

    	echo json_encode(array('ggtop'=>$ggtop,'shop'=>$shop,'bottom'=>$bottom,'pro'=>$pro));
    	exit();
    }
    /**
     * [getlist 加载更多]

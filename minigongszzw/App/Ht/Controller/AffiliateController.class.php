...
class AffiliateController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('affiliate')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$affiliate=M('affiliate')->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('affiliate')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["photo_d"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $dt = M('affiliate')->where('pid='.intval($id))->field('photo_d')->find();
					    if ($id && $dt['photo_d']) {
			...
			//执行添加
			if(intval($id)>0){
				$imgs = M('affiliate')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('affiliate')->where('id='.intval($id))->save($array);
				
				
			}else{
				$sql = M('affiliate')->add($array);
		...
		// 查询信息
		//=========================
		$affiliate= $id>0 ? M('affiliate')->where('id='.intval($id))->find() : array();
		//获取所有商品轮播图
		if($affiliate){
			if ($affiliate['photo_string']) {
				$img_str = explode(',', trim($affiliate['photo_string'],','));
	...
	*/
	public function img_del(){
		...
		$check_info = M('affiliate')->where('id="'.intval($id).'"')->find();
		if (!$check_info) {
		...
		if (in_array($img_url, $arr)) {
			...
			$res = M('affiliate')->where('id='.intval($id))->save($data);
			if (!$res) {
	...
	{
		$id = intval($_REQUEST['id']);
		$info = M('affiliate')->where('id='.intval($id))->find();
		if (!$info) {
		...
		$order_id = M('order_product')->where('name="'.$info['name'].'"')->getField('order_id');
		if($order_id){
			$status = M('order')->where('id="'.intval($order_id).'"')->getField('status');
			if(intval($status) == 20){
				$this->error('删除失败，该产品下还有未发货订单！');
				exit();
			}
		}
		$up = M('affiliate')->where('id="'.$id.'"')->delete();
		if ($up) {

...
class SubsidyController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('subsidy')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$subsidy=M('subsidy')->limit($limit,rows)->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('subsidy')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["photo_d"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $dt = M('subsidy')->where('id='.intval($id))->field('photo_d')->find();
					    if ($id && $dt['photo_d']) {
		 ...
		 //  	$up_arr = array();
			...
			//执行添加
			if(intval($id)>0){
				// $imgs = M('subsidy')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('subsidy')->where('id='.intval($id))->save($array);
				$sql2 = M('subsidy_more')->where('sub_id="'.intval($_REQUEST['id']).'"')->delete();
				...
				if($title){
					foreach($title as $k => $v){
						if($v){
							...
							$tmp['sub_id'] = $_REQUEST['id'];
							M('subsidy_more')->add($tmp);
						}
		...
		// 查询信息
		//=========================
		$subsidy= $id>0 ? M('subsidy')->where('id='.intval($id))->find() : array();
		$content = $id>0 ? M('subsidy_more')->where('sub_id='.intval($id))->select() : array();
		//获取所有商品轮播图
		if($subsidy){
			if ($subsidy['photo_string']) {
				$img_str = explode(',', trim($subsidy['photo_string'],','));

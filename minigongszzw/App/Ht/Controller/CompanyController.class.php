...
class CompanyController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('company_register')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$company=M('company_register')->limit($limit,rows)->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('company_register')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["process"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('company_register')->where('id='.intval($id))->field('process')->find();
					    if ($id && $xt['process']) {
			...
			//执行添加
			if(intval($id)>0){
				$imgs = M('company_register')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('company_register')->where('id='.intval($id))->save($array);
				$sql2 = M('company_adv')->where('comp_id="'.intval($_REQUEST['id']).'"')->delete();
				...
				if($title){
					foreach($title as $k => $v){
						if($v){
							$tmp['title'] = $v;
							$tmp['content'] = $content[$k];
							$tmp['comp_id'] = $_REQUEST['id'];
							M('company_adv')->add($tmp);
						}
				...
				$sql = M('company_register')->add($array);
		...
		// 查询信息
		//=========================
		$company= $id>0 ? M('company_register')->where('id='.intval($id))->find() : array();
		$adv = $id>0 ? M('company_adv')->where('comp_id='.intval($id))->select() : array();
		//获取所有商品轮播图
		if($company){
			if ($company['photo_string']) {
				$img_str = explode(',', trim($company['photo_string'],','));

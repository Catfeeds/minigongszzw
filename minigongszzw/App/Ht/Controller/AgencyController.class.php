...
class AgencyController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('agency')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$agency=M('agency')->limit($limit,rows)->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('agency')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["process"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('agency')->where('id='.intval($id))->field('process')->find();
					    if ($id && $xt['process']) {
			...
			//上传产品小图
			if (!empty($_FILES["server_process"]["tmp_name"])) {
					//文件上传
					$info = $this->upload_images($_FILES["server_process"],array('jpg','png','jpeg'),"product/".date(Ymd));
				    if(!is_array($info)) {// 上传错误提示错误信息
				        $this->error($info);
				        exit();
				    }else{// 上传成功 获取上传文件信息
					    $array['server_process'] = 'UploadFiles/'.$info['savepath'].$info['savename'];
					    $xt = M('agency')->where('id='.intval($id))->field('server_process')->find();
					    if ($id && $xt['server_process']) {
					    	$img_url = "Data/".$xt['server_process'];
							if(file_exists($img_url)) {
								@unlink($img_url);
							}
					    }
			...
			//上传产品小图
			if (!empty($_FILES["get_server"]["tmp_name"])) {
					//文件上传
					$info = $this->upload_images($_FILES["get_server"],array('jpg','png','jpeg'),"product/".date(Ymd));
				    if(!is_array($info)) {// 上传错误提示错误信息
				        $this->error($info);
				        exit();
				    }else{// 上传成功 获取上传文件信息
					    $array['get_server'] = 'UploadFiles/'.$info['savepath'].$info['savename'];
					    $xt = M('agency')->where('id='.intval($id))->field('get_server')->find();
					    if ($id && $xt['get_server']) {
					    	$img_url = "Data/".$xt['get_server'];
							if(file_exists($img_url)) {
								@unlink($img_url);
							}
					    }
			...
			if (!empty($_FILES["photo_d"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $dt = M('company_register')->where('pid='.intval($id))->field('photo_d')->find();
					    if ($id && $dt['photo_d']) {
			...
			//执行添加
			if(intval($id)>0){
				// $imgs = M('agency')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('agency')->where('id='.intval($id))->save($array);

				
			}else{
				$sql = M('agency')->add($array);
		...
		// 查询信息
		//=========================
		$agency= $id>0 ? M('agency')->where('id='.intval($id))->find() : array();
	...
	*/
	public function img_del(){
		...
		$check_info = M('company_register')->where('id="'.intval($company_id).'"')->find();
		if (!$check_info) {
		...
		if (in_array($img_url, $arr)) {
			...
			$res = M('company_register')->where('id='.intval($company_id))->save($data);
			if (!$res) {

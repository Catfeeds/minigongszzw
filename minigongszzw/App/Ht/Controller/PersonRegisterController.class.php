...
class PersonRegisterController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('person_register')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$person=M('person_register')->limit($limit,rows)->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('person_register')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["photo_d"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $dt = M('person_register')->where('pid='.intval($id))->field('photo_d')->find();
					    if ($id && $dt['photo_d']) {
			...
			//执行添加
			if(intval($id)>0){
				$imgs = M('person_register')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('person_register')->where('id='.intval($id))->save($array);
				
			}else{
				$sql = M('person_register')->add($array);
		...
		// 查询信息
		//=========================
		$person= $id>0 ? M('person_register')->where('id='.intval($id))->find() : array();
		//获取所有商品轮播图
		if($person){
			if ($person['photo_string']) {
				$img_str = explode(',', trim($person['photo_string'],','));
	...
	*/
	public function img_del(){
		...
		$check_info = M('person_register')->where('id="'.intval($id).'"')->find();
		if (!$check_info) {
		...
		if (in_array($img_url, $arr)) {
			...
			$res = M('person_register')->where('id='.intval($id))->save($data);
			if (!$res) {

...
class ApproveController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('special_approval')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$approve=M('special_approval')->limit($limit,rows)->select();
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('special_approval')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			if (!empty($_FILES["photo_d"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $dt = M('special_approval')->where('pid='.intval($id))->field('photo_d')->find();
					    if ($id && $dt['photo_d']) {
			...
			//执行添加
			if(intval($id)>0){
				$imgs = M('special_approval')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('special_approval')->where('id='.intval($id))->save($array);
				
			}else{
				$sql = M('special_approval')->add($array);
		...
		// 查询信息
		//=========================
		$approve= $id>0 ? M('special_approval')->where('id='.intval($id))->find() : array();
		//获取所有商品轮播图
		if($approve){
			if ($approve['photo_string']) {
				$img_str = explode(',', trim($approve['photo_string'],','));
	...
	*/
	public function img_del(){
		...
		$check_info = M('special_approval')->where('id="'.intval($id).'"')->find();
		if (!$check_info) {
		...
		if (in_array($img_url, $arr)) {
			...
			$res = M('special_approval')->where('id='.intval($id))->save($data);
			if (!$res) {

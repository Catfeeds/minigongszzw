...
class ShebaoController extends PublicController{
	...
	public function index(){
		$where = '';
		define('rows',20);
		$count=M('shebao')->count();
		$rows=ceil($count/rows);
		$page=(int)$_GET['page'];
		$page<0?$page=0:'';
		$limit=$page*rows;
		$page_index=$this->page_index($count,$rows,$page);
		$shebao=M('shebao')->limit($limit,rows)->select();
		
		//=============
		// 将变量输出
		//=============	
		$this->assign('shebao',$shebao);
		$this->display();
	}
	//**********************************************
	//说明：产品 添加修改
	...
	public function add(){	
		...
		try{
			...
			if (!empty($_FILES["photo_x"]["tmp_name"])) {
				    ...
				    if(!is_array($info)) {// 上传错误提示错误信息
					    ...
					    $xt = M('shebao')->where('id='.intval($id))->field('photo_x')->find();
					    if ($id && $xt['photo_x']) {
			...
			// 		    $dt = M('shebao')->where('pid='.intval($id))->field('photo_d')->find();
			// 		    if ($id && $dt['photo_d']) {
			...
			//执行添加
			if(intval($id)>0){
				$imgs = M('shebao')->where('id='.intval($id))->getField('photo_string');
				...
				$sql = M('shebao')->where('id='.intval($id))->save($array);
				
			}else{
				$sql = M('shebao')->add($array);
		...
		// 查询信息
		//=========================
		$shebao= $id>0 ? M('shebao')->where('id='.intval($id))->find() : array();
		//获取所有商品轮播图
		if($shebao){
			if ($shebao['photo_string']) {
				$img_str = explode(',', trim($shebao['photo_string'],','));
	...
	*/
	public function img_del(){
		...
		$check_info = M('shebao')->where('id="'.intval($id).'"')->find();
		if (!$check_info) {
		...
		if (in_array($img_url, $arr)) {
			...
			$res = M('shebao')->where('id='.intval($id))->save($data);
			if (!$res) {
